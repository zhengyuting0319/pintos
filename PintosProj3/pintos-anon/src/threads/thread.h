...
typedef int mapid_t; //typedef the mapid_t  
...
struct thread
  {
   ...
   struct list children; 
    ...
    struct thread *parent;
    /*indictae the status of child loading status
    0 : have not been loaded 
    -1 : fail
    1 : success
   ...
   // int is_exit;
   int excute_subtract_wait; /* avoid wait-twice */
   // struct lock child_lock;
   

   struct semaphore initial_sema; /* sema between start and excute */
   struct semaphore wait_sema; /* sema in wait, for parent and child */
...
// to indicate the child thread status
